...
function initializeCharts() {
    ...
    const ctxRegioes = document.getElementById("barchart-regioes").getContext("2d");
    window.chartInstanceRegioes = new Chart(ctxRegioes, createBarChartConfig("Regioes"));
}
...
function loadInitialData() {
    ...
    document.getElementById("barchart-regioes").parentElement.style.display = "none";
            ...
            axios
                ...
                .then((response) => {
                    ...
                    if (selectedFilter === "bairros") {
                        ...
                        document.getElementById("regiao-filter-container").style.display = "none";
                    }
                })
                .catch((error) => {
...
function loadRegioesSubfilters() {
    axios
        .get("/get-regioes")
        .then(response => {
            const subfilterSelect = document.getElementById('regiao-subfilter-select');
            subfilterSelect.innerHTML = '<option value="">Selecione uma Região</option>';

            response.data.forEach(regiao => {
                const option = document.createElement('option');
                option.value = regiao; // O valor é diretamente o nome da região
                option.textContent = regiao; // O texto é o nome da região
                subfilterSelect.appendChild(option);
            });

            document.getElementById('regiao-subfilter-container').style.display = 'block';

            subfilterSelect.addEventListener('change', handleRegiaoSubfilterChange);
        })
        .catch(error => {
...
function handleRegiaoSubfilterChange(event) {
    const selectedRegiao = event.target.value; // Captura o nome ou ID da região selecionada
    ...
    if (selectedRegiao) {
        axios
            ...
            .then((response) => {
                updateChartInstance(window.chartInstanceRegioes, response.data, 'regioes');
            })
            .catch((error) => {
                console.error("Erro ao buscar dados para a região:", error);
            });
    } else {
...
function updateChartInstance(chartInstance, data, filter) {
    ...
    if (!Array.isArray(data) || data.length === 0) {
        ...
        if (filter === "partidos") {
            ...
            // Para regiões, o array é simples com valores de 'regiao'
            chartInstance.data.labels = data; // Diretamente os valores das regiões
            chartInstance.data.datasets[0].data = data.map(() => 0);
        } else {
